...
public class CapstoneMainController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		InitialArray(); // 3개의 ArrayList를 초기화 하는 작업
		
		if(drawCanvas.isResizable() == true)
	...
	public void InitialArray()
	{
		...
		for(int i=0;i<5;i++)
		{
			normalBlocks.add(new NumberBlock(i,Color.BLUE,Integer.toString(i)));
		}
			// NormalBlock에 대한 코드.
	...
	public void setPanelBlocks(ArrayList<Block> input)
	{
		...
		for(int i=0;i<input.size();i++)
		{
			...
			PanelBox.setMargin(panelButton, new Insets(10,10,10,10)); // top, bottom, right, left
